...
const { INVENTORY } = require('../constants/auditActions');
...
const consumeInventoryForPrescriptionItems = async (items = []) => {
...
const getInventoryAudit = async (filters = {}) => {
  const query = buildInventoryQuery(filters);
...
const createInventoryItem = async (payload, actor = null) => {
  ...
  if (actor) {
    logAudit({
      ...
      module: 'inventory',
      resource: { collection: 'Inventory', docId: item._id },
      diff: { after: item.toObject() }
    });
  }
...
const updateInventoryItem = async (id, payload, actor = null) => {
  ...
  const before = existingItem.toObject();
...
const deleteInventoryItem = async (id, actor = null) => {
  ensureObjectId(id, 'inventory id');
  ...
  if (actor) {
    logAudit({
      ...
      module: 'inventory',
      resource: { collection: 'Inventory', docId: id },
      diff: { before: item.toObject() }
    });
  }
...
module.exports = {
  consumeInventoryForPrescriptionItems,
  getInventoryStatus: resolveInventoryStatus,
  syncInventoryAlertsForItem: createAlertFromInventoryItem,
  getInventoryRiskSummary: getInventoryRiskFlags,
  listInventory,
  getInventoryAudit,
  createInventoryItem,
  updateInventoryItem,
