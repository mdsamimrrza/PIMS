...
import { getApiMessage, listAlerts, listInventory, listPrescriptions, createPharmacyOrder } from '../../api/pimsApi';
...
export default function PharmacistDashboard() {
  ...
  const incomingPrescriptions = prescriptions.filter((item) => {
    const matchStatus = item.status === 'Pending' || item.status === 'Processing' || item.status === 'Emergency_Draft';
    const matchType = pageState.typeFilter === 'all' || item.prescriptionType === pageState.typeFilter;
    ...
    <section className="page">
      ...
      <div className="content-grid-2">
        <section className="panel">
          ...
          <div className="button-group mb-4" style={{ display: 'flex', gap: '8px', marginBottom: '1rem' }}>
            {['all', 'opd', 'ipd', 'emergency', 'tto'].map(t => (
              <button 
                key={t}
                className={`button-${pageState.typeFilter === t ? 'primary' : 'ghost'} button-small`}
                onClick={() => setPageState(p => ({ ...p, typeFilter: t }))}
              >
                {t.toUpperCase()}
              </button>
            ))}
          ...
          <div className="mini-list">
            {incomingPrescriptions.map((entry) => (
              ...
              >
                <Link
                  to={`/patients/${entry.patientId?._id}/details`}
                ...
                <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.5rem' }}>
                  <span className={`status-pill`} style={{ 
                    background: entry.priority === 'emergency' ? 'var(--danger)' : (entry.priority === 'stat' ? '#f97316' : (entry.priority === 'urgent' ? '#f59e0b' : 'var(--surface-muted)')),
                    color: 'white', border: 'none'
                  }}>
                    {entry.priority?.toUpperCase() || 'ROUTINE'}
                  </span>
                  {entry.status === 'Emergency_Draft' && (
                    <div className="status-pill status-warning" style={{ fontSize: '0.7rem' }}>Pending Sign-off</div>
                  )}
                        ...
                        try {
                          await createPharmacyOrder({
                            ...
                            items: entry.items.map(i => ({
                              ...
                              batchId: 'B-MAIN-01', // Example batch
                              quantityDispensed: i.quantity || 1
                            }))
                          });
                          alert('Prescription Dispensed and Inventory Updated!');
